...
Example (from the repository root):
    python scripts/feature_engineering/rank_stock_characteristics.py --input data/raw/stock_characteristics.parquet
...
def choose_characteristics(
    ...
    if columns is not None:
        ...
        non_numeric = [
            column for column in columns if not pd.api.types.is_numeric_dtype(panel[column])
        ]
        if non_numeric:
